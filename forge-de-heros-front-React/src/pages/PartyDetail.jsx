...
function PartyDetail() {
  ...
  useEffect(() => {
    const fetchParty = async () => {
      try {
        ...
        // normaliser la réponse : accepter { data: {...} } ou {...}
  ...
  // currentSize preferred, fallback sur members.length
  ...
  const isFull = party.isFull ?? remainingSpots <= 0;
          ...
          {party.members && party.members.length > 0 ? (
            ...
            <div>
              <p className="pd-muted">
                Les membres ne sont pas listés pour ce groupe.
              </p>
              <p>
                Nombre actuel de membres : <strong>{currentMembers}</strong>
              </p>
            </div>
          )}
        </section>
